...
- **Frente C — Telas internas pós-feedback UX.** Doc fonte: `docs/CORA_Telas_Internas_Pendencias.md`. 5 sub-itens distribuídos pelas 4 telas internas, exige discussão prévia por item antes de virar briefing técnico.
  - **Item 1 (hierarquia da Home) — em curso (12/05/2026).** Decisões fechadas em `docs/CORA_FrenteC_HomeHierarquia_Decisoes.md`. Briefing técnico v2 em `docs/CORA_Briefing_FrenteC_Item1_Home.md` (cesta como carrinho persistido com confirmação; tabela `weekly_orders` + 3 endpoints + cron de aviso de abandono via Vercel Cron). Prompt operacional em `docs/CORA_Prompt_FrenteC_Item1_ClaudeCode.md`. Env var nova `CRON_SECRET` configurada no Vercel (Production + Preview) pré-implementação. Estrutura: 2 PRs (`feat/weekly-orders-backend` → `feat/home-hierarchy-redesign`) com 3 pontos de parada pra revisão. Pendente: mandar pro Claude Code.
