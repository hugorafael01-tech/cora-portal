...
const D={
  ...
  pães:[
    {nome:"Pão Original",peso:"550g",preco:"R$ 24,50",precoNum:24.5,img:IMG.original,desc:"Fermentação natural, casca crocante, miolo macio.",ingredientes:"Farinha de trigo, água, sal, levain da Cora.",detalhe:"Fermentação longa de 36h. Apenas 4 ingredientes. Crosta firme, miolo aberto com alvéolos irregulares.",qtd:1},
    {nome:"Pão Integral",peso:"550g",preco:"R$ 24,50",precoNum:24.5,img:IMG.integral,desc:"100% integral, sementes de linhaça e girassol.",ingredientes:"Farinha integral, água, sal, levain, linhaça, girassol.",detalhe:"100% farinha integral. Mesma fermentação longa, com sementes tostadas que dão crocância.",qtd:0},
    {nome:"Multi Grãos",peso:"650g",preco:"R$ 31,50",precoNum:31.5,img:IMG.multigraos,desc:"Aveia, centeio, gergelim e mel.",ingredientes:"Farinha de trigo, centeio, aveia, água, mel, sal, levain, gergelim.",detalhe:"Cinco grãos na massa, mel na fermentação. Miolo denso, casca com gergelim tostado.",qtd:0},
    {nome:"Brioche",peso:"450g",preco:"R$ 37,00",precoNum:37,img:IMG.brioche,desc:"Manteiga francesa, textura amanteigada.",ingredientes:"Farinha, manteiga, ovos, açúcar, sal, levain, leite.",detalhe:"Massa enriquecida com manteiga. Fermentação 18h. Miolo dourado, textura que desfia.",qtd:0},
  ],
  hist:[
...
const Home=({onNav,pending,confirmed,addPending,removePending,updateConfirmed})=>{
  ...
  const handleAdd=p=>{addPending(p);setModal(null);setToastMsg(`${p.nome} adicionada. Confirme no Cardápio.`);setToast(true);setTimeout(()=>setToast(false),5000);};
  const handleQtyChange=(product,delta)=>{if(confirmed.length>0){if(delta>0)updateConfirmed(addTo(confirmed,product));else updateConfirmed(removeFrom(confirmed,product.nome));}else{if(delta>0)addPending(product);else removePending(product.nome);}};
...
const Cardapio=({pending,confirmed,setPending,setConfirmed,hasPending})=>{
  ...
  const removeItem=n=>{const pi=pending.findIndex(p=>p.nome===n);if(pi!==-1){setPending(prev=>removeFrom(prev,n));return;}const ci=confirmed.findIndex(p=>p.nome===n);if(ci!==-1)setConfirmed(prev=>removeFrom(prev,n));};
