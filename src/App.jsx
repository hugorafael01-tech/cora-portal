...
/* CORA — Portal do Assinante — Protótipo v3.2.2
   Correções PO: modal qty-aware, chevron expand, proporcional,
   prazo 3a 22h, confirmação de pedido, empty states, aria-labels */
...
// Simulated deadline logic: terça 22h
const isPastDeadline = () => {
  const now = new Date();
  const day = now.getDay(); // 0=dom,1=seg,2=ter,3=qua,4=qui,5=sex,6=sab
  const hour = now.getHours();
  // Past deadline: after terça 22h (day>2, or day===2 && hour>=22) until quinta 22h (day<4, or day===4 && hour<22)
  // Simplified for prototype: always false (toggle to test)
  return false;
};
  ...
  return<>
    ...
    <div role="dialog" aria-label={`Detalhes: ${product.nome}`} style={{position:"fixed",bottom:0,left:0,right:0,maxWidth:390,margin:"0 auto",background:"#FFF",borderRadius:"16px 16px 0 0",zIndex:51,maxHeight:"85vh",overflowY:"auto",boxShadow:"0 -4px 24px rgba(26,24,21,0.12)",animation:"slideUp 300ms ease"}}>
      <div style={{position:"relative"}}>
        <Photo h={200} color={product.cor||"#D5C8B5"} accent={!!product.historia} label={product.nome}/>
      ...
      <div style={{padding:20}}>
        ...
        {/* Qty-aware: se já adicionou, mostra −/+ em vez de botão */}
        <div style={{marginTop:4}}>
          {hasQty ? (
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 0"}}>
              <div style={{fontFamily:fb,fontSize:14,color:ST.success.t,fontWeight:500,display:"flex",alignItems:"center",gap:6}}>
                <I d={ic.check} size={16} color={ST.success.t}/>Na sua cesta
              </div>
              <QtyBtn qty={qty} onAdd={onAdd} onRemove={onRemove} name={product.nome}/>
            </div>
          ) : (
            <>{onAction&&<Btn primary full onClick={onAction} ariaLabel={actionLabel}>{actionLabel}</Btn>}{hint&&<div style={{fontFamily:fb,fontSize:12,color:W[500],textAlign:"center",marginTop:8}}>{hint}</div>}</>
          )}
        </div>
      </div>
    </div>
...
// ─── NOVIDADE CARD ───
const NovidadeCard=({extra,qty,onCardClick,onAdd,onRemove})=><Card style={{padding:0,overflow:"hidden",cursor:"pointer",marginBottom:16}} onClick={onCardClick} ariaLabel={`Novidade: ${extra.nome}`}><Photo h={200} accent label={extra.nome}/><div style={{padding:16}}><SL t="Novidade da semana"/><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12}}><div style={{flex:1}}><div style={{fontFamily:fb,fontSize:18,fontWeight:600,color:W[800]}}>{extra.nome}</div><div style={{fontFamily:fb,fontSize:14,color:W[600],marginTop:4}}>{extra.preco} — Só assinantes</div></div>{qty===0?<button onClick={e=>{e.stopPropagation();onCardClick();}} className="bp" aria-label={`Conhecer ${extra.nome}`} style={{padding:"10px 24px",borderRadius:8,border:"none",background:B[500],color:"#FFF",fontFamily:fb,fontSize:14,fontWeight:500,cursor:"pointer",minHeight:44,flexShrink:0}}>Quero</button>:<QtyBtn qty={qty} onAdd={()=>{if(onAdd)onAdd();}} onRemove={()=>{if(onRemove)onRemove();}} name={extra.nome}/>}</div></div></Card>;

// ─── DEADLINE WARNING ───
const DeadlineWarning=({pastDeadline})=>(
  <div style={{fontFamily:fb,fontSize:12,color:pastDeadline?ST.warning.t:ST.warning.t,background:pastDeadline?"#FFEDD5":ST.warning.bg,padding:"8px 12px",borderRadius:8,marginBottom:20,display:"inline-flex",alignItems:"center",gap:8,border:`1px solid ${pastDeadline?"#FDBA74":ST.warning.b}`}}>
    <I d={ic.clock} size={14} color={pastDeadline?"#9A3412":ST.warning.t}/>
    {pastDeadline
      ? "O prazo desta semana já passou. Pedidos feitos agora serão entregues na próxima quinta."
      : "Pedidos até terça, 22h, para entrega na quinta"}
  </div>
);

// ═══ HOME ═══
const Home=({onNav,pedidos,addP,removeP})=>{
  const[modal,setModal]=useState(null);
  const[toast,setToast]=useState(false);
  const[toastMsg,setToastMsg]=useState("");
  const cnt=n=>pedidos.filter(p=>p.nome===n).length;
  const handleAdd=p=>{addP(p);setModal(null);setToastMsg(`${p.nome} adicionada. Confirme no Cardápio.`);setToast(true);setTimeout(()=>setToast(false),5000);};
  ...
  return<div style={{padding:"24px 16px 16px"}}>
    ...
    {/* Novidade hero */}
    {D.extras.length>0?
      <NovidadeCard extra={D.extras[0]} qty={cnt(D.extras[0].nome)} onCardClick={()=>setModal(D.extras[0])} onAdd={()=>addP(D.extras[0])} onRemove={()=>removeP(D.extras[0].nome)}/>
    ...
    <div onClick={()=>onNav("cardapio")} className="lk" role="link" aria-label="Ver cardápio completo" style={{fontFamily:fb,fontSize:14,color:B[500],fontWeight:500,textAlign:"center",padding:"8px 0",cursor:"pointer"}}>Ver cardápio completo ›</div>

    {/* Modal qty-aware */}
    {modal&&<Modal product={modal} onClose={()=>setModal(null)} onAction={()=>handleAdd(modal)} actionLabel="Adicionar à cesta" hint="Cobrado na próxima fatura" qty={cnt(modal.nome)} onAdd={()=>addP(modal)} onRemove={()=>removeP(modal.nome)}/>}
...
const Assinatura=({onNav})=>{
  ...
  const proporcional=Math.abs(diff/4*D.semanasRestantes);
  ...
  return<div style={{padding:"24px 16px 16px"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      <h2 style={{fontFamily:fd,fontSize:26,textTransform:"uppercase",color:B[800],margin:0}}>Sua Assinatura</h2>
      <Badge label="Ativa"/>
    </div>
    ...
    <Card style={{marginBottom:12}}>
      ...
      <div style={{display:"flex",gap:12,alignItems:"center"}}>
        <SmPhoto w={56} h={48} color="#E8D5B7"/>
        <div style={{flex:1}}>
          <div style={{fontFamily:fb,fontSize:16,fontWeight:600,color:W[800]}}>{D.cesta.nome}</div>
          <div style={{fontFamily:fb,fontSize:13,color:W[600]}}>{D.cesta.itens}</div>
          <div style={{fontFamily:fb,fontSize:15,fontWeight:600,color:B[500],marginTop:4}}>{D.cesta.valor}</div>
        </div>
      </div>

      {!editing&&<Btn full onClick={()=>setEditing(true)} style={{marginTop:12}} ariaLabel="Alterar minha cesta">Alterar minha cesta</Btn>}

      {editing&&<div style={{marginTop:16,borderTop:`1px solid ${W[200]}`,paddingTop:16}}>
        <DeadlineWarning pastDeadline={isPastDeadline()}/>
        ...
        {D.pães.map((p,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:i<D.pães.length-1?`1px solid ${W[100]}`:"none"}}>
          <div style={{flex:1}}>
            <div style={{fontFamily:fb,fontSize:14,fontWeight:600,color:W[800]}}>{p.nome} <span style={{fontWeight:400,fontSize:12,color:W[500]}}>({p.peso})</span></div>
            <div style={{fontFamily:fb,fontSize:12,color:W[600],marginTop:4}}>{p.preco}/un</div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <button aria-label={`Remover ${p.nome}`} onClick={()=>upd(i,-1)} disabled={qtds[i]===0} className="qb" style={{width:32,height:32,borderRadius:8,border:`1px solid ${qtds[i]>0?W[300]:W[200]}`,background:"none",cursor:qtds[i]>0?"pointer":"default",fontSize:18,color:qtds[i]>0?W[600]:W[300],display:"flex",alignItems:"center",justifyContent:"center",opacity:qtds[i]===0?0.4:1}}>−</button>
            <span style={{fontFamily:fb,fontSize:16,fontWeight:600,color:qtds[i]>0?B[500]:W[400],width:24,textAlign:"center"}}>{qtds[i]}</span>
            <button aria-label={`Adicionar ${p.nome}`} onClick={()=>upd(i,1)} disabled={total>=3} className="qb" style={{width:32,height:32,borderRadius:8,border:`1px solid ${total<3?B[500]:W[200]}`,background:total<3?B[50]:"none",cursor:total<3?"pointer":"default",fontSize:18,color:total<3?B[500]:W[300],display:"flex",alignItems:"center",justifyContent:"center",opacity:total>=3?0.4:1}}>+</button>
          </div>
        </div>)}
        ...
        {changed&&<>
          <div style={{fontFamily:fb,fontSize:12,color:W[500],textAlign:"right",marginBottom:4}}>Antes: {fmt(orig)} → Depois: {fmt(mensal)}</div>
          
          {/* Proporcional com chevron expand */}
          <div onClick={()=>setShowCalc(!showCalc)} style={{background:B[50],borderRadius:8,padding:"10px 12px",marginBottom:12,cursor:"pointer",border:`1px solid ${B[100]}`}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{fontFamily:fb,fontSize:13,color:B[700]}}>
                Ajuste neste mês: <span style={{fontWeight:600}}>{diff>0?"+":""}{fmt(proporcional)}</span>
              </div>
              <I d={ic.chevDown} size={16} color={B[400]} style={{transform:showCalc?"rotate(180deg)":"none",transition:"transform 200ms ease"}}/>
            </div>
            {showCalc&&<div style={{marginTop:8,fontFamily:fb,fontSize:12,color:B[600],lineHeight:1.6,animation:"fadeUp 200ms ease"}}>
              Faltam {D.semanasRestantes} semanas neste mês.<br/>
              Diferença semanal: {diff>0?"+":""}{fmt(Math.abs(diff/4))}/semana<br/>
              Cobrado proporcionalmente na próxima fatura: {diff>0?"+":""}{fmt(proporcional)}
            </div>}
          </div>
        </>}

        <div style={{display:"flex",gap:8}}>
          <Btn onClick={()=>{setEditing(false);setQtds(D.pães.map(p=>p.qtd));setShowCalc(false);}} style={{flex:1}} ariaLabel="Cancelar alteração">Cancelar</Btn>
          <Btn primary disabled={!changed} onClick={handleSave} style={{flex:2}} ariaLabel="Salvar alterações da cesta">{changed?"Salvar":"Faça uma alteração"}</Btn>
        </div>
      </div>}
    </Card>
    ...
    <Card onClick={()=>onNav("perfil")} style={{marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}} ariaLabel="Ver histórico de entregas e cobranças"><div style={{display:"flex",alignItems:"center",gap:12}}><I d={ic.file} size={20} color={B[500]}/><div><div style={{fontFamily:fb,fontSize:14,fontWeight:500,color:W[700]}}>Histórico de entregas e cobranças</div><div style={{fontFamily:fb,fontSize:12,color:W[500]}}>{D.cob.mes} — {D.cob.valor} — {D.cob.status}</div></div></div><I d={ic.chev} size={16} color={W[400]}/></Card>
  </div>;
};

// ═══ CARDÁPIO ═══
const Cardapio=({pedidos,setP})=>{
  const[exp,setExp]=useState(null);
  const[modal,setModal]=useState(null);
  const[confirmed,setConfirmed]=useState(false);
  const pastDL=isPastDeadline();
  const cnt=n=>pedidos.filter(p=>p.nome===n).length;
  const addP=p=>{const pn=parseFloat(p.preco.replace("R$ ","").replace(",","."));setP(prev=>[...prev,{nome:p.nome,preco:p.preco,precoNum:pn}]);};
  const remP=n=>setP(prev=>{const i=prev.findIndex(p=>p.nome===n);if(i===-1)return prev;return[...prev.slice(0,i),...prev.slice(i+1)];});
  const totalP=pedidos.reduce((s,p)=>s+p.precoNum,0);

  const handleConfirm=()=>{
    setConfirmed(true);
    setTimeout(()=>{setP([]);setConfirmed(false);},4000);
  };
  ...
  return<div style={{padding:"24px 16px 80px"}}>
    ...
    <DeadlineWarning pastDeadline={pastDL}/>

    {/* Novidade */}
    {D.extras.length>0?D.extras.map((ex,i)=><NovidadeCard key={i} extra={ex} qty={cnt(ex.nome)} onCardClick={()=>setModal(ex)} onAdd={()=>addP(ex)} onRemove={()=>remP(ex.nome)}/>):<Card style={{marginBottom:16,padding:20,textAlign:"center"}}><div style={{fontFamily:fb,fontSize:14,color:W[500]}}>Nenhuma novidade esta semana.</div></Card>}
    ...
    {D.pães.map((p,i)=>{const q=cnt(p.nome);const isExp=exp===i;return<Card key={i} style={{marginBottom:8}}>
      <div style={{display:"flex",gap:12,alignItems:"center"}}>
        <SmPhoto w={64} h={52} color={p.cor} onClick={()=>setExp(isExp?null:i)} style={{cursor:"pointer",borderRadius:8}}/>
        <div onClick={()=>setExp(isExp?null:i)} style={{flex:1,cursor:"pointer"}}>
          <div style={{fontFamily:fb,fontSize:14,fontWeight:600,color:W[800]}}>{p.nome} <span style={{fontWeight:400,fontSize:12,color:W[500]}}>({p.peso})</span></div>
          <div style={{fontFamily:fb,fontSize:12,color:W[500],marginTop:4,lineHeight:1.4}}>{p.desc}</div>
          <div style={{display:"flex",alignItems:"center",gap:8,marginTop:4}}>
            <span style={{fontFamily:fb,fontSize:12,color:W[600]}}>{p.preco}/un</span>
            {/* Chevron down hint */}
            <I d={ic.chevDown} size={14} color={W[400]} style={{transform:isExp?"rotate(180deg)":"none",transition:"transform 200ms ease"}}/>
          </div>
        </div>
        {q===0?<button onClick={()=>addP(p)} className="bp" aria-label={`Pedir ${p.nome}`} style={{padding:"8px 16px",borderRadius:8,border:"none",background:B[500],color:"#FFF",fontFamily:fb,fontSize:12,fontWeight:500,cursor:"pointer",flexShrink:0,minHeight:40}}>Pedir</button>:<QtyBtn qty={q} onAdd={()=>addP(p)} onRemove={()=>remP(p.nome)} name={p.nome}/>}
      </div>
      {isExp&&<div style={{marginTop:12,paddingTop:12,borderTop:`1px solid ${W[200]}`,animation:"fadeUp 200ms ease"}}>
        ...
        <div style={{fontFamily:fb,fontSize:13,color:W[600],lineHeight:1.5,marginBottom:12}}>{p.ingredientes}</div>
        ...
        <div style={{fontFamily:fb,fontSize:13,color:W[700],lineHeight:1.6}}>{p.detalhe}</div>
      </div>}
    </Card>;})}

    {/* Rodapé com confirmação */}
    {pedidos.length>0&&!confirmed&&<div style={{position:"fixed",bottom:56,left:0,right:0,maxWidth:390,margin:"0 auto",background:"#FFF",borderTop:`1px solid ${W[200]}`,padding:"12px 16px",zIndex:5}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div>
          <div style={{fontFamily:fb,fontSize:12,color:W[600]}}>{pedidos.length} {pedidos.length===1?"item":"itens"} — extras desta semana</div>
          <div style={{fontFamily:fb,fontSize:15,fontWeight:600,color:W[800]}}>{fmt(totalP)}</div>
        </div>
        <Btn primary onClick={handleConfirm} ariaLabel="Confirmar pedido">Confirmar</Btn>
      </div>
      <div style={{fontFamily:fb,fontSize:11,color:W[500],marginTop:4}}>Além da sua assinatura. Cobrado na próxima fatura.</div>
    </div>}

    {/* Confirmação sucesso */}
    {confirmed&&<div style={{position:"fixed",bottom:56,left:0,right:0,maxWidth:390,margin:"0 auto",background:ST.success.bg,borderTop:`1px solid ${ST.success.b}`,padding:"16px",zIndex:5,textAlign:"center",animation:"fadeUp 300ms ease"}}>
    ...
    {/* Modal qty-aware */}
    {modal&&<Modal product={modal} onClose={()=>setModal(null)} onAction={()=>{addP(modal);setModal(null);}} actionLabel="Adicionar à cesta" hint="Cobrado na próxima fatura" qty={cnt(modal.nome)} onAdd={()=>addP(modal)} onRemove={()=>remP(modal.nome)}/>}
  </div>;
};

// ═══ PERFIL ═══
const Perfil=()=>{const[cpf,setCpf]=useState(false);const dados=[["Endereço","Ed. Boa Vista, Bl. A / 502"],["Dia de entrega","Quintas-feiras"],["WhatsApp","(21) 99876-5432"],["E-mail","beatriz@email.com"],["CPF",cpf?"123.456.789-00":"•••.•••.789-00"]];return<div style={{padding:"24px 16px 16px"}}><h2 style={{fontFamily:fd,fontSize:26,textTransform:"uppercase",color:B[800],margin:"0 0 20px"}}>Perfil</h2><div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}><div style={{width:48,height:48,borderRadius:9999,background:B[50],display:"flex",alignItems:"center",justifyContent:"center",fontFamily:fd,fontSize:20,color:B[500],textTransform:"uppercase"}}>B</div><div><div style={{fontFamily:fb,fontSize:16,fontWeight:600,color:W[800]}}>Beatriz Silva</div><div style={{fontFamily:fb,fontSize:12,color:W[500]}}>beatriz@email.com</div></div></div><Card style={{marginBottom:12}}><SL t="Dados pessoais"/>{dados.map(([l,v],i)=><div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 0",borderBottom:i<dados.length-1?`1px solid ${W[100]}`:"none"}}><div><div style={{fontFamily:fb,fontSize:11,color:W[400],marginBottom:2}}>{l}</div><div style={{fontFamily:fb,fontSize:13,color:W[700]}}>{v}</div></div>{l==="CPF"?<button aria-label={cpf?"Ocultar CPF":"Mostrar CPF"} onClick={()=>setCpf(!cpf)} style={{background:"none",border:"none",cursor:"pointer",padding:4,minWidth:44,minHeight:44,display:"flex",alignItems:"center",justifyContent:"center"}}><I d={cpf?ic.eyeOff:ic.eye} size={16} color={W[400]}/></button>:<I d={ic.chev} size={14} color={W[400]}/>}</div>)}</Card><Card style={{marginBottom:12}}><SL t="Histórico de entregas e cobranças"/><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}><I d={ic.check} size={14} color={ST.success.t}/><span style={{fontFamily:fb,fontSize:13,fontWeight:500,color:ST.success.t}}>Tudo em dia</span></div>{D.hist.map((h,i)=><div key={i} style={{padding:"12px 0",borderBottom:i<D.hist.length-1?`1px solid ${W[100]}`:"none"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontFamily:fb,fontSize:12,color:W[400]}}>{h.sem}</span><Badge label={h.st} type={h.st==="Entregue"?"success":"info"}/></div><div style={{fontFamily:fb,fontSize:13,color:W[700],marginTop:4}}>{h.itens}</div>{h.extra&&<div style={{fontFamily:fb,fontSize:12,color:B[500],marginTop:4}}>+ {h.extra.nome} — {h.extra.valor}</div>}</div>)}<div style={{marginTop:12,padding:12,borderRadius:8,background:W[50],border:`1px solid ${W[200]}`}}><div style={{fontFamily:fb,fontSize:12,color:W[400],marginBottom:4}}>Cobrança do mês</div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontFamily:fb,fontSize:14,fontWeight:600,color:W[800]}}>{D.cob.mes} — {D.cob.valor}</span><Badge label="Pago"/></div></div><div style={{marginTop:8,padding:12,borderRadius:8,background:B[50],border:`1px solid ${B[100]}`}}><div style={{fontFamily:fb,fontSize:12,color:B[700],marginBottom:4}}>Próxima fatura (abril)</div><div style={{fontFamily:fb,fontSize:13,color:B[800],lineHeight:1.6}}>Assinatura: R$ 98,00<br/>+ Extras: R$ 22,00<br/>+ Frete: R$ 15,00<br/><span style={{fontWeight:600}}>= R$ 135,00 (estimado)</span></div></div></Card><Card style={{marginBottom:12}}><SL t="Cartão"/><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontFamily:fb,fontSize:14,fontWeight:500,color:W[800]}}>{D.cartao.band} ••••{D.cartao.n}</div><div style={{fontFamily:fb,fontSize:12,color:W[500],marginTop:4}}>Próxima: {D.cartao.prox}</div></div><Btn style={{padding:"8px 16px",fontSize:12}} ariaLabel="Atualizar cartão">Atualizar</Btn></div></Card><Card style={{marginBottom:12}}><div style={{fontFamily:fb,fontSize:14,color:W[700],marginBottom:12,lineHeight:1.5}}>Precisa pausar ou cancelar? Fale pelo WhatsApp.</div><button className="bw" aria-label="Falar pelo WhatsApp" style={{width:"100%",padding:"12px 0",borderRadius:8,background:"#25D366",color:"#FFF",border:"none",fontFamily:fb,fontSize:14,fontWeight:500,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,minHeight:44}}><I d={ic.msg} size={18} color="#FFF"/>Falar pelo WhatsApp</button></Card><button className="bl" aria-label="Sair da conta" style={{width:"100%",padding:"12px 0",borderRadius:8,background:"none",color:W[500],border:`1px solid ${W[300]}`,fontFamily:fb,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,minHeight:44}}><I d={ic.logout} size={16} color={W[500]}/>Sair da conta</button></div>;};

// ═══ APP ═══
export default function CoraPortal(){
  const[scr,setScr]=useState("home");
  const[ped,setPed]=useState([]);
  const addP=p=>{const pn=parseFloat(p.preco.replace("R$ ","").replace(",","."));setPed(prev=>[...prev,{nome:p.nome,preco:p.preco,precoNum:pn}]);};
  const remP=n=>setPed(prev=>{const i=prev.findIndex(p=>p.nome===n);if(i===-1)return prev;return[...prev.slice(0,i),...prev.slice(i+1)];});
  return<div style={{fontFamily:fb,maxWidth:390,margin:"0 auto",background:W[50],minHeight:"100vh",display:"flex",flexDirection:"column"}}>
    ...
    <div style={{flex:1,overflowY:"auto"}}>
      {scr==="home"&&<Home onNav={setScr} pedidos={ped} addP={addP} removeP={remP}/>}
      {scr==="assinatura"&&<Assinatura onNav={setScr}/>}
      {scr==="cardapio"&&<Cardapio pedidos={ped} setP={setPed}/>}
      {scr==="perfil"&&<Perfil/>}
    </div>
    <Nav active={scr} onNav={setScr} badge={ped.length}/>
