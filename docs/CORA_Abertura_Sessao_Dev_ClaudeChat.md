...
Nunca assumir que não tenho acesso a um repo, ferramenta ou dado. Testar primeiro (clonar o repo, rodar tool_search). Os repos da Cora são clonáveis por leitura, sem auth. Se eu me pegar dizendo "não tenho acesso a X", isso é o gatilho pra tentar acessar X, não pra seguir sem ele.
...
Quando uma sessão produzir um erro de processo novo (assumi algo sem verificar, modelei da fonte errada, pulei o pre-flight), virar uma entrada nova em "Anti-padrões nomeados" com data e o caso concreto.
