...
import { executaGeracao } from "../api/_lib/geracao-runner.js";
import { baseAtual } from "../api/_lib/asaas.js";
...
async function uniqueDePagamentoJaCaiu() {
  ...
  const { error } = await supabaseAdmin.from("faturas").insert(sonda);
  // Limpa a sonda de qualquer jeito, tenha ela entrado ou nao.
  await supabaseAdmin.from("faturas").delete().eq("periodo_referencia", "2099-01");
...
try {
  ...
  if (criouAlgo) {
    console.log("\nlimpando as faturas do periodo...");
    try {
